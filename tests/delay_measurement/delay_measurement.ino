...
// Struct to store delay measurements
struct delays_t {
  TickType_t t0;
  TickType_t delta;
};
...
void setup() {
  ...
  while (xQueueReceive(samples_queue, &q, portMAX_DELAY)) {
    ...
    if (Leq_samples >= SAMPLE_RATE * LEQ_PERIOD) {
      ...
      delays.delta = xTaskGetTickCount() - delays.t0;
      delays.t0 = xTaskGetTickCount();
      ...
      if (USE_SOFTWARE_SERIAL == 0) Serial.printf("%.1f %s\n delta_t: %u\n\n", Leq_dB, DB_UNITS, delays.delta);
      else Serial.printf("%.1f %s\n delta_t: %u\n\n", Leq_dB, DB_UNITS, delays.delta);
